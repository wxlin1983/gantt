...
| 2. 排程引擎 | 完成 |
| 3. Case 核心 API | 完成 |
| 4. Gantt 視覺化 | 完成（自建 SVG 雙軌 bar、DAG 連線、緩衝區塊、phase 分組、虛擬化用的佈局數學已備但尚未接上捲動） |
| 5. 執行引擎 | 完成 |
...
- **WebSocket 即時推送**（§8.8）：尚未實作，前端目前靠 TanStack Query 的 staleTime 與視窗聚焦重取。
- **流程 D（在兩 task 間插入 task）**：服務層與 API 尚未實作，設計文件 §8.4 的規格已定。
